...
user_plays <- read_excel("~/Intro data science/MUSIC PROJECT/user_plays_with_song_ID.xlsx")
head(user_plays)
head(newmusic)
colnames(user_plays)<-c("song.id","play_count","User_count")
user_plays2 <- user_plays
newmusic2 <- newmusic
merge(x = newmusic2, y = user_plays2, by = "song.id", all.x = TRUE)
newmusic3 <- newmusic2[c(1:5,15:17,22,24:25,34:36)]
cmbomusic<-merge(x = newmusic3, y = user_plays2, by = "song.id", all.x = TRUE)
head(cmbomusic)
...
cmbomusic3 <- cmbomusic3[-c(2:4,12)]
...
rf2 <- randomForest(cmbomusic4[,-7:-8],cmbomusic4[,8])
rf2
```
## Results
